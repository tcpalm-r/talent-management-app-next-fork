...
import { analyzeReviewConfig, buildAnalyzeReviewPrompt } from '@/lib/prompts';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = buildAnalyzeReviewPrompt({ employeeName, reviewText });

    console.log('[analyze-review API] Calling Claude API...');

    const response = await anthropic.messages.create({
      model: analyzeReviewConfig.model,
      max_tokens: analyzeReviewConfig.maxTokens,
      messages: [
        {
          role: 'user',
          content: prompt,
    ...
    const fallbackResult: ReviewAnalysis = {
      ...
      insights: {
        ...
        developmentAreas: [
          'Clarify growth plan with manager',
          'Expand cross-functional influence',
          'Document repeatable processes',
        ],
        successMetrics: ['Hit next cycle KPIs', 'Complete all plan action items', 'Capture progress update in 90 days'],
